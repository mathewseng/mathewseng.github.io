...
In Custom mode, displayed indexes are source-calibrated against published Hi-Lo index tables. Earlier builds displayed every raw EV crossover generated by the browser's simplified count-to-card-weight model. That approximation produced plausible decision directions but materially wrong thresholds, such as `15 vs T` appearing near `+1.5` instead of the published `+4`, so the visible chart no longer treats those raw crossovers as authoritative deviations.
