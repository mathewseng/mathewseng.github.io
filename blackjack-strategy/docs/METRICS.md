...
Hard-total chart rows are calculated from a weighted mix of the possible non-pair two-card compositions for that total, instead of using a single representative hand such as `T6` for every hard 16. The exact hand solver still removes the actual cards selected by the user.

The Index Group menu can show published Hi-Lo groups directly, including Illustrious 18, Fab 4, Sweet 16, Catch 20, Catch 22, BJA S17/H17, and JackAce MD S17. Selecting one of those groups replaces the generated Custom deviations with the published overlay. The chart settings area includes an audit strip that compares generated Custom crossover points against the selected source group, or against BJA S17/H17 when Custom is selected. The visible range slider defaults to -5 through +10; the number boxes can be typed beyond that slider span when a wider filter is needed. Displayed index values can be rounded from 0 to 3 decimal places.
