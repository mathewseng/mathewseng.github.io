...
The Index Group menu can also show published Hi-Lo groups, including Illustrious 18, Fab 4, Sweet 16, Catch 20, Catch 22, BJA S17/H17, and JackAce MD S17. The chart settings area includes an audit strip that compares generated Custom crossover points against the selected source group, or against BJA S17/H17 when Custom is selected. The visible range slider defaults to -5 through +10; the number boxes can be typed beyond that slider span when a wider filter is needed. Displayed index values can be rounded from 0 to 3 decimal places.

Published source rows are kept separate from Custom. Selecting a named group replaces chart deviations with that group. Selecting Custom keeps the generated EV crossover values and uses the source rows only for the audit summary.
