...
In Custom mode, raw indexes are calculated from EV crossover points using a Hi-Lo-constrained shoe model. The rank probabilities preserve visible dead-card removal, then solve a maximum-entropy card mix whose expected Hi-Lo tag balance moves by `-trueCount / 52` per card. That is the missing step from earlier builds: the old code used an arbitrary exponential tilt, which made `15 vs T` appear near `+1.5`; the current raw model puts that crossover near the published `+4`.

Known published cells in Custom mode are anchored to the rule-appropriate BJA S17/H17 playable index after the raw crossover is computed. This fixes cases such as `16 vs T`, where the raw representative-hand crossover can appear near `+1` even though the accepted Hi-Lo playing index is `0`.

The Index Group menu can also show published Hi-Lo groups directly, including Illustrious 18, Fab 4, Sweet 16, Catch 20, Catch 22, BJA S17/H17, and JackAce MD S17. The chart settings area includes an audit strip that compares generated Custom crossover points against the selected source group, or against BJA S17/H17 when Custom is selected. The visible range slider defaults to -5 through +10; the number boxes can be typed beyond that slider span when a wider filter is needed. Displayed index values can be rounded from 0 to 3 decimal places.
